"""Tests for 12-1 momentum entry signal."""
...
class SyntheticPriceDataAccess:
    """DataAccess stub backed by a deterministic price series."""
...
def test_momentum_12_1_computes_trailing_return_over_trading_days() -> None:
    ...
    assert results[0].raw_signal_value is not None
    assert results[0].raw_signal_value > Decimal("0")


def test_momentum_12_1_excludes_insufficient_history() -> None:
    ...
    signal = Momentum12_1EntrySignal()
    results = signal.calculate(evaluation_date, snapshot, data_access)

    assert results == []


def test_momentum_12_1_respects_point_in_time_filtering() -> None:
    security_id = SecurityId("SEC_PIT")
    start_date = date(2020, 1, 1)
    bars = _build_daily_prices(
        security_id=security_id,
        start_date=start_date,
        day_count=400,
        start_price=Decimal("100"),
        daily_multiplier=Decimal("1.002"),
    )
    evaluation_date = start_date + timedelta(days=350)
    future_bar = PriceBar(
        trade_date=evaluation_date + timedelta(days=5),
        open=Decimal("999"),
        high=Decimal("999"),
        low=Decimal("999"),
        close=Decimal("999"),
    ...
    data_access = SyntheticPriceDataAccess({security_id: [*bars, future_bar]})
    snapshot = _membership_snapshot(
        evaluation_date,
        [
            UniverseMembership(
...
def test_signal_factory_builds_momentum_12_1() -> None:
    signal = build_entry_signal(
        SignalConfig(
            name="momentum_12_1",
            params={"lookback_days": 126, "skip_days": 10},
        )
    )

    assert isinstance(signal, Momentum12_1EntrySignal)
    assert signal.lookback_days == 126
    assert signal.skip_days == 10
    assert signal.missing_data_policy is MissingDataPolicy.EXCLUDE_SECURITY


def test_momentum_12_1_on_mag7_fixture() -> None:
    ...
    assert all(result.signal_id == "momentum_12_1" for result in results)
